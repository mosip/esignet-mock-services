                ...
                <tbody className="w-full text-[#031640]">
                    {applicationsList.length > 0 && applicationsList.map((eachItem, index) => (
                        <tr key={index} className="border-b border-[#E3DCC9]">
                            <td className="py-5 text-left w-[19%] pl-6"><p className="flex items-center"><img onClick={() => selectApplication(eachItem.userInfoId)} src={`/assets/icons/${selectedItems.includes(eachItem.userInfoId) ? 'checkbox-active' : 'checkbox'}.svg`} alt="checkbox" className="h-[18px] w-[18px] align-middle cursor-pointer"/><span className="pl-4">{eachItem.nationalUid}</span></p></td>
                            <td className="py-5 text-left w-[19%]">{eachItem.firstNamePrimary}</td>
                            <td className="py-5 text-left w-[13%]">{eachItem.lastNameSecondary}</td>
                            <td className="py-5 text-left w-[16%]">{new Date(eachItem.issuanceDate).toLocaleDateString('en-GB').replaceAll('/', '-')}</td>
