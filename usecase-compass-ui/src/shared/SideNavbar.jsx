...
const Drawer = () => {
    ...
    if(location.pathname !== '/editApplication' && localStorage.getItem("editFormData")){
        ...
        <div className={`${fullDrawer ? "w-[266px]" : "w-[117px]"} drawer py-7 px-3 transition-all duration-300`} style={{ boxShadow: '2px 0 2px rgba(0, 0, 0, 0.1)' }}>
            ...
            <div className="drawer-side">
                ...
                <ul className="menu bg-base-200 text-base-content min-h-full py-4 px-2 text-[14px]">
                    ...
                    <li><Link to="/newApplication" className={`flex items-center space-x-3 mt-5 ${location.pathname === '/newApplication' || location.pathname === '/editApplication' ? "text-[#622F17]" : "text-[#9C9B98]"}`}><img src={location.pathname === '/newApplication' || location.pathname === '/editApplication' ? "/assets/icons/newApplication.svg" : "/assets/icons/newApplication-gray.svg" } alt="newApp" className="h-[48px] w-[48px]"/>{fullDrawer && <span className="opacity-0 animate-[fadeIn_0.1s_ease-in-out_.2s_forwards]">New Application</span>}</Link></li>
                </ul>
            </div>
        </div>
    )
