...
const EsimStepperForm = ({ userInfo, address, onSubmitSuccess }) => {
  ...
  const info = [];
  if (userInfo) {
    info.push({ label: t('personalInfo.fullName'), value: userInfo.name || null });
    info.push({ label: t('personalInfo.mobileNumber'), value: userInfo.phone_number || null });
    info.push({ label: t('personalInfo.email'), value: userInfo.email || null });
    info.push({ label: t('personalInfo.gender'), value: userInfo.gender || null });
    info.push({ label: t('personalInfo.dateOfBirth'), value: userInfo.birthdate || null });
    if (address) {
            ...
            >
              <div className="flex items-start">
                ...
                <div className={`pl-[9px] font-semibold text-gray-700`}>
                  <div className="font-bold text-sm leading-4 uppercase tracking-[0.16px] text-[#696B6F]">{t('stepper.stepLabel', { step: step })}</div>
                  ...
                  <>
                    {!userInfo ? (
                      ...
                      <div className="flex flex-col items-center justify-center min-h-[250px] text-gray-500">
                        ...
                        <p className="text-lg font-medium">{t('loading.userDetails', 'Loading User Details...')}</p>
                      </div>
                    ) : (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 sm:gap-x-8 gap-y-4 my-4">
                          ...
                          {userInfo.picture && (
                            <div className="col-span-1 sm:col-start-2 sm:row-start-1 sm:row-span-2">
                              <div className="flex items-center gap-2">
                                <label className="text-xs sm:text-sm font-medium text-gray-700 block">{t('personalInfo.photoLabel', 'Photo')}</label>
                                <div className="relative group">
                                  <img src="/images/tick.svg" alt="Verified Icon" className="w-4 h-4" />
                              ...
                              <img src={userInfo.picture} alt={t('personalInfo.userPhotoAlt', 'User Photo')} className="w-32 h-32 rounded-md object-cover border-2 border-gray-200 shadow-md mt-1" />
                            </div>
                          )}
                          ...
                          {info.map((field) => (
                            <div className="col-span-1" key={field.label}>
                              <div className="flex items-center gap-2">
                                <label className="text-xs sm:text-sm font-medium text-gray-700 block">{field.label}</label>
                                {field.value && (
                                  <div className="relative group">
                                    <img src="/images/tick.svg" alt="Verified Icon" className="w-4 h-4" />
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-xs font-medium rounded py-1 px-2 z-10">
                        ...
                        <div className="flex justify-end mt-6">
                          <button
                            className={`w-full h-[48px] lg:w-[381px] border-[1px] rounded-[8px] font-medium ${!isFormComplete ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 border-[#017DC0] text-white hover:bg-blue-700'}`}
