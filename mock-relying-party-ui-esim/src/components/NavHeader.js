        ...
        <div className="flex items-center gap-5 ml-auto rtl:ml-0 rtl:mr-auto order-2 rtl:order-1 md:order-2">
          ...
          <div className="relative inline-block" ref={dropdownRef}>
            <div className="flex items-center gap-1 ml-1 rtl:flex-row-reverse md:ml-0 rtl:mr-0 rtl:gap-1.5">
              ...
              <button
                id="language-toggle"
                className="flex items-center gap-1 font-semibold text-base text-gray-800 py-1 border-none rounded bg-transparent cursor-pointer transition-colors duration-200 min-w-20 justify-between pr-6 md:pr-3 md:py-2 md:min-w-18"
                onClick={() => setLanguageDropdownOpen(!languageDropdownOpen)}
              >
                <span>{currentLanguageName}</span>
                <svg
                  className={`transition-transform duration-200 flex-shrink-0 relative top-[2px] ${
            ...
            {languageDropdownOpen && (
              <div className="absolute top-full -left-15 rtl:right-auto rtl:left-0 bg-white border border-gray-200 rounded-lg shadow-lg min-w-58 z-[1000] mt-1 py-2 md:-right-2.5 md:-left-2.5 ltr:md:!-left-[6.5rem] before:content-[''] before:absolute before:-top-1.5 before:left-1/2 before:transform before:-translate-x-1/2 before:rotate-45 before:w-3 before:h-3 before:bg-white before:border-l before:border-t before:border-gray-200 before:-z-10">
